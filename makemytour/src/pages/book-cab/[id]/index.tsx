...
import { getcab, handlecabbooking } from "@/api";
...
const BookCabPage = () => {
  ...
  if (cabs.length === 0) {
    return <div>No cab data available for this ID.</div>;
  }
  const cab = cabs[0];
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setQuantity(
      isNaN(value) ? 1 : Math.max(1, Math.min(value, cab.availableSeats))
    );
  };

  const totalPrice = cab?.price * quantity;
  ...
  const handlebooking = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const data = await handlecabbooking(
        user?.id,
        cab?.id,
        quantity,
        grandTotal
      ...
      <div className="grid gap-6 mt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor="cabType" className="flex items-center">
              <CarIcon className="w-4 h-4 mr-2" />
              Cab Type
            </Label>
            <Input id="cabType" value={cab?.cabType} readOnly />
          </div>
          <div className="space-y-2">
            ...
            <Input id="from" value={cab?.from} readOnly />
          </div>
          <div className="space-y-2">
            ...
            <Input id="to" value={cab?.to} readOnly />
          </div>
          <div className="space-y-2">
            ...
            <Input
              id="departureTime"
              value={new Date(cab.departureTime).toLocaleString()}
              readOnly
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              id="arrivalTime"
              value={new Date(cab.arrivalTime).toLocaleString()}
              readOnly
            />
          </div>
          <div className="space-y-2">
